...
// predfind(20) -> predfind(left, 6) -> predfind(left(2), 6) |
//                                                          -> predfind(left(2)-right(1), 6) -> NULL
//
...
pair<Node *, Node *> split(Node *T, int k)
{
  ...
  if(T == NULL) return make_pair(leftRoot, rightRoot);
    ...
    //if(result.first != NULL ) T->size -= result.first->size;
...
{ 
  ...
  /*
  cout << "Here's the head element: " << T->key << endl;
  Node *n = T;
  while(n != NULL) {
    cout << "Stepping down the left: " << n->key << endl;
    n = n->left;
  }
  */
